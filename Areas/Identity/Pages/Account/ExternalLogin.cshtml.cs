    ...
    public class ExternalLoginModel : PageModel
    {
        ...
        public async Task<IActionResult> OnPostConfirmationAsync(string returnUrl = null)
        {
            ...
            if (ModelState.IsValid)
            {
                ...
                if (result.Succeeded)
                {
                    var roleResult = await _userManager.AddToRoleAsync(user, "Consumidor");
        ...
        private IdentityUser CreateUser()
        {
            ...
            catch
            {
                throw new InvalidOperationException($"Não foi possível criar uma instância de '{nameof(IdentityUser)}'. " +
                    $"Verifique se '{nameof(IdentityUser)}' não é uma classe abstrata e possui construtor sem parâmetros.");
            }
        }
        ...
        private IUserEmailStore<IdentityUser> GetEmailStore()
        {
            ...
            {
                throw new NotSupportedException("O UI padrão requer suporte a email no UserStore.");
            }
